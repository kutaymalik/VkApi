...
public class CreateMoneyTransferValidator : AbstractValidator<MoneyTransferRequest>
{
    ...
    {
        RuleFor(x => x.FromAccountId).NotEmpty().GreaterThan(0).WithMessage("FromAccountId is required.");
        RuleFor(x => x.ToAccountId).NotEmpty().GreaterThan(0).WithMessage("ToAccountId is required.");
